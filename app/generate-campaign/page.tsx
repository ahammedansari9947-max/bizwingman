...
export default function GenerateCampaignPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 md:py-12 space-y-10 overflow-visible">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[350px_1fr] gap-6 items-start">
        <div className="lg:sticky lg:top-8 w-full">
          <Card className="saas-card h-auto">
            ...
            <CardContent className="p-6">
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-1.5">
                  ...
                  <Input id="businessName" name="businessName" placeholder="e.g. Malabar Spice Co." value={formData.businessName} onChange={handleChange} className="saas-input h-10 text-sm w-full" required />
                </div>
                
                <div className="space-y-1.5">
                  <Label htmlFor="businessType" className="text-sm font-semibold text-muted-foreground dark:text-gray-300">Industry</Label>
                  <Input id="businessType" name="businessType" placeholder="e.g. Spices Export, Restaurant" value={formData.businessType} onChange={handleChange} className="saas-input h-10 text-sm w-full" required />
                </div>
                
                <div className="space-y-1.5">
                  <Label htmlFor="campaignGoal" className="text-sm font-semibold text-muted-foreground dark:text-gray-300">Campaign Goal</Label>
                  <Input id="campaignGoal" name="campaignGoal" placeholder="e.g. Increase Onam sales" value={formData.campaignGoal} onChange={handleChange} className="saas-input h-10 text-sm w-full" required />
                </div>
                ...
                <div className="pt-2">
                  <Button type="submit" className="w-full h-10 text-sm font-bold saas-button-primary" disabled={loading}>
                    {loading ? (
                      <>
        ...
        <div className="w-full flex-1">
          {loading ? (
            <div className="h-[500px] flex flex-col items-center justify-center space-y-6 saas-card bg-secondary/10 border-border transition-all duration-700 w-full">
              ...
              <div className="text-center space-y-2 px-6">
                ...
                <p className="text-sm text-muted-foreground max-w-sm mx-auto">Our AI is analyzing your brand and market trends to build the perfect strategy.</p>
              </div>
            </div>
          ) : results ? (
            <CampaignResults data={results} />
          ) : (
            <div className="h-[500px] flex flex-col items-center justify-center saas-card bg-white dark:bg-white/[0.02] border-border text-muted-foreground p-8 w-full">
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-sm max-w-xs mx-auto">Complete the form to unlock your tailored 7-day marketing blueprint.</p>
              </div>
            </div>
          )}
        </div>
